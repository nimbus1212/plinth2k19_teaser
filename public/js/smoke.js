...
function init() {
    for (stats = new Stats, stats.setMode(0), stats.domElement.style.position = "absolute", stats.domElement.style.left = "0px", stats.domElement.style.top = "0px", clock = new THREE.Clock, (renderer = new THREE.WebGLRenderer).setSize(window.innerWidth, window.innerHeight), scene = new THREE.Scene, (camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 1e4)).position.z = 1e3, scene.add(camera), geometry = new THREE.CubeGeometry(200, 200, 200), material = new THREE.MeshLambertMaterial({
        ...
        }), text = new THREE.Mesh(textGeo, textMaterial), screen.width >= 1920 ? (text.position.z = 800, text.position.x = 15, text.position.y = -70) : screen.width >= 1330 && screen.width <= 1919 ? (text.position.z = 800, text.position.x = 10, text.position.y = -100) : screen.width <= 1329 && screen.width >= 950 ? 1280 === screen.width ? (text.position.z = 800, text.position.x = 15, text.position.y = -90) : (text.position.z = 800, text.position.x = 10, text.position.y = -95) : screen.width <= 949 && screen.width >= 779 ? (text.position.z = 800, text.position.x = 0, text.position.y = -120) : screen.width <= 769 && screen.width >= 625 ? screen.width > screen.height ? (text.position.z = 800, text.position.x = 0, text.position.y = -90) : (text.position.z = 800, text.position.x = 0, text.position.y = -90) : screen.width <= 624 && screen.width >= 300 && (screen.width > screen.height ? (text.position.z = 800, text.position.x = 0, text.position.y = -90) : (text.position.z = 800, text.position.x = 0, text.position.y = -50)), setTimeout(function () {
            scene.add(text)
